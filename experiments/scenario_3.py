...
#initialization
with open(os.path.join(os.path.dirname(__file__), '..', 'config', 'scenario_3.yaml')) as f:
    config = yaml.safe_load(f)
r=config["seed"]
Js=config["nums_workers"]
alpha=config["alpha"]
...
concurrencies_dict:Dict[int,List]=config["concurrencies_dict"]
step_size:float=config["step_size"] # the step size for update theta
...
for J in Js:
    ...
    result_dir = os.path.join(os.path.dirname(__file__), '..', 'results', 'scenario_3', f'Num_w_{J}')
    os.makedirs(result_dir, exist_ok=True)
    concurrencies=concurrencies_dict[J]
